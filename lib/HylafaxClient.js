...
HylafaxClient.prototype.getCompletedJobs = function(cb){
  this._sendList('doneq', cb);
}

HylafaxClient.prototype.getDocs = function(cb){
  this._sendList('docq', cb);
}
...
HylafaxClient.prototype._sendList = function(type, cb){
  var list = [];
  this.ftp.list(type, function(err, res){
    res.on('success', function(){
      cb(null, list);
    })
    res.on('raw', function(data){
      list.push(data);
    })
  })
}
...
HylafaxClient.prototype.sendPort = function(port_a, port_b,cb){
  ...
  this.ftp.send('PORT', port_host.join(',') ,function(err, response){
    cb(err, response);
  })
}

HylafaxClient.prototype.sendType = function(type,cb){
  this.ftp.send('TYPE', 'I' ,function(err, response){
    cb(err, response);
  })
}

HylafaxClient.prototype.sendMode = function(mode,cb){
  this.ftp.send('MODE', 'Z' ,function(err, response){
    cb(err, response);
  })
}

HylafaxClient.prototype.sendFax = function(stream, number, callback){
  ...
  actions.push(function(cb){    
    self.ftp.temp_put(stream.pipe(zlib.createDeflate()), function(err, res){
  ...
  actions.push(function(cb){
    self.ftp.send('JPARM', 'DIALSTRING "' + number + '"', function(err, response){    
      cb(null, response);
    })
  })
